...
async def test_train_from_db_insufficient_data():
    mock_db = AsyncMock()
    with patch("app.services.trainer.build_training_rows", new=AsyncMock(return_value=[])):
...
async def test_train_from_db_success(mock_joblib_dump):
    ...
    with patch("app.services.trainer.build_training_rows", new=AsyncMock(return_value=rows)):
