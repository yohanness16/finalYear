...
async def cleanup_raw_telemetry(db: AsyncSession, batch_size: int = 10000) -> int:
    ...
    cutoff = datetime.now(UTC) - timedelta(days=settings.RAW_TELEMETRY_RETENTION_DAYS)
...
async def cleanup_trip_history(db: AsyncSession, batch_size: int = 5000) -> int:
    ...
    settings = get_settings()
    cutoff = datetime.now(UTC) - timedelta(days=settings.TRIP_HISTORY_RETENTION_DAYS)
    result = await db.execute(
